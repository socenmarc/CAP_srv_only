...
function splitBatchResponse(response) {
    ...
    try {
        var boundary = getBoundary(header_builder_1.getHeaderValue('content-type', response.headers));
        return splitResponse(body, boundary);
    }
    catch (err) {
        throw util_1.errorWithCause('Could not parse batch response.', err);
    }
}
...
function splitChangeSetResponse(changeSetResponse) {
    ...
    try {
        var boundary = getBoundary(header_builder_1.getHeaderValue('content-type', headers));
        return splitResponse(changeSetResponse, boundary);
    }
    catch (err) {
        throw util_1.errorWithCause('Could not parse change set response.', err);
    }
}
...
function parseBatchResponse(batchResponse) {
    return splitBatchResponse(batchResponse).map(function (response) {
        var contentType = header_builder_1.getHeaderValue('content-type', parseHeaders(response));
