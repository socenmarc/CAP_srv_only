...
function destinationForServiceBinding(serviceInstanceName, options) {
    ...
    var destination = options.transformationFn
        ...
        : transform(selected);
    return destination &&
        proxy_util_1.proxyStrategy(destination) === proxy_util_1.ProxyStrategy.INTERNET_PROXY
...
function flattenServiceBindings(vcapServices) {
    return util_1.flatten(Object.values(vcapServices));
}
...
function searchServiceBindingForDestination(name) {
    ...
    try {
        ...
        logger.info("Could not retrieve destination from service binding. If you are not using SAP Extension Factory, this information probably does not concern you. " + error.message);
    }
}
