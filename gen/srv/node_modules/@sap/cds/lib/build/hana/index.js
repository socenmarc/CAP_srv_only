    ...
    async _stripCsvComments(csvFiles) {
        ...
        for (const file of csvFiles) {
            if (this.isStagingBuild() || file.startsWith(this.task.options.compileDest)) {
                const tmpName = randomBytes(16).toString('base64').replace(/\//g, '_')
                const tmpFile = path.join(path.dirname(file), tmpName)
                await CSV.stripComments(fs.createReadStream(file), fs.createWriteStream(tmpFile))
