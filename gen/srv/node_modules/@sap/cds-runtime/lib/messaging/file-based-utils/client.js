...
const _useLines = (lines, subscriptions, executeArray, writeArray) => {
  for (const line of lines) {
    ...
    try {
      ...
      const msg = { event, headers, data }
      if (subscriptions.has(msg.event)) executeArray.push(msg)
      else writeArray.push(line)
    } catch (e) {
      console.log(e)
    }
  }
...
const emit = async (req, file, lock) => {
  ...
  if (!('id' in headers)) headers.id = generateUUID()
  const msg = Object.assign(headers, { data: req.data, event: req.event })
  const line = '\n' + JSON.stringify(msg)
