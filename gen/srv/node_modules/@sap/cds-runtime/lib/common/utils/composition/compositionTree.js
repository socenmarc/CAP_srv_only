...
const _resolvedElement = element => {
  ...
  const skipForbiddenViewCheck = element.type === 'cds.Association' && element.on && !element['@odata.contained']
...
const _getCompositionTree = ({
  ...
  let rootName = resolveViews ? _resolvedEntityName(rootEntityName, definitions) : rootEntityName
...
const _addToOneKey = (data, link, toOneKeys) => {
  if (data[link.targetKey] !== undefined) {
    toOneKeys[link.entityKey] = data[link.targetKey]
  }
}
  ...
  for (const toOneElement of toOneElements) {
    if (toOneElement.name in subDataEntry) {
      ...
      for (const link of links) {
        _addToOneKey(toOneData, link, toOneKeys)
      }
    } else {
      ...
      for (const backLink of backLinks) {
        ...
        _addToOneKey(data, backLink, toOneKeys)
      }
    }
  }
  return toOneKeys
...
const _selectDeepUpdateData = ({
  ...
  if (data !== undefined) {
    ...
    _dataElements(entity).forEach(element => {
      if (
        ...
        ((includeAllRootColumns && root) || data.find(entry => element.name in entry))
      ) {
  ...
  return execute(cqn2cqn4sql(selectCQN, { definitions }))
    .then(result => {
...
const _isSameEntity = (definitions, cqn, context) => {
  ...
  const target = getDBTable(context.target)
...
const selectDeepUpdateData = (definitions, cqn, execute, context, includeAllRootColumns = false) => {
  ...
  if (context && _isSameEntity(definitions, sqlQuery, context)) {
  ...
  return _selectDeepUpdateData({
    ...
    execute,
    includeAllRootColumns,
    singleton: context && context.target && isSingleton(context.target),
    alias
  })
}
