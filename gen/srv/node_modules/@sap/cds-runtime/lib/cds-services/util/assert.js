...
const _checkCreateUpdate = (result, ref, rootEntity, checks, data, context, run) => {
  ...
  return data.reduce((result, row) => {
    ...
    // REVISIT: Why don't we check referential integrity for assoc with on condition?
    if (resolvedElement.on) {
      return result
    }
...
const _filterAssocs = (element, structuredAssocs, prefix = []) => {
  ...
  return (
    element.type === 'cds.Association' &&
    !element['@odata.contained'] &&
    element._target['@cds.persistence.skip'] !== true &&
    ...
    result = Object.keys(csn.definitions)
      .filter(
        key =>
          ...
          // skip check for aspects and localized tables
