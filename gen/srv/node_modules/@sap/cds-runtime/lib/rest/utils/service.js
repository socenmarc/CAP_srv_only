...
const { getTransition, revertData } = require('../../common/utils/resolveView')
...
const _pathToReqOptions = (method, path, data, srvname) => {
  const reqOptions = {
    method: method,
    url: path.startsWith('/') ? path : path.startsWith(`${srvname}.`) ? path.replace(`${srvname}.`, '/') : '/' + path
  }
  if (data && Object.keys(data).length) reqOptions.data = data
  return reqOptions
}
...
const getReqOptions = (req, query, service) => {
  const resolvedTarget = getTransition(req.target, true).target
  const reqOptions =
    typeof query === 'object'
      ? _cqnToReqOptions(query, resolvedTarget, service.kind)
      : typeof query === 'string'
        ? _stringToReqOptions(query, req.data)
        : _pathToReqOptions(req.method, req.path, req.data, service.name)
