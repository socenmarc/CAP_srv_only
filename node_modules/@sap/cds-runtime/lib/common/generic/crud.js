...
module.exports = function () {
  this.on(['CREATE', 'READ', 'UPDATE', 'DELETE'], '*', async req => {
    ...
    // case: no authorization and payload more than just keys but no changes -> affected rows === 0 -> no change or not exists?
    if (req.event === 'UPDATE' && result === 0 && (await _targetEntityDoesNotExist(req))) {
      req.reject(404)
    }
