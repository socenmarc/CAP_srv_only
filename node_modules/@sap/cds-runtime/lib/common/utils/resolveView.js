...
const _checkForForbiddenViews = queryTarget => {
  ...
  if (select) {
    if (!select.from || select.from.join || select.from.length > 1) {
      throw getError(501, 'NON_WRITABLE_VIEW', [_event || 'INSERT|UPDATE|DELETE'])
    }
    if (select.where) {
      DEBUG && DEBUG(`Ignoring where clause during ${_event || 'INSERT|UPDATE|DELETE'} on view "${queryTarget.name}"`)
    }
  }
}
...
const _getTransitionData = (target, columns, force, skipForbiddenViewCheck) => {
  ...
  if (_isProjection(target, force)) {
    return _getTransitionData(target.query._target, columns, undefined, skipForbiddenViewCheck)
  }
