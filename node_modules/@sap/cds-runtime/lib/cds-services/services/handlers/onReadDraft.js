...
const _draftAdminTable = context => {
...
const _allInactive = (context, columns, model) => {
  const table = {
  ...
  const pk = Object.keys(model.definitions[context.target.name.replace(/_texts$/, '')].keys).find(
...
const _generateCQN = (context, columns, serviceName, model) => {
  ...
  if (
    ...
    context.target.query._target.name === 'DRAFT.DraftAdministrativeData'
  ) {
    return _draftAdminTable(context)
  }
  ...
  if (!draftParameters.isActiveEntity) {
    if (_draftInSubSelect(context.query.SELECT.where)) {
      return _allInactive(context, columns, model)
    }
    return _allActive(context, columns)
  }
