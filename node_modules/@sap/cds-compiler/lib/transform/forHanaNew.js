...
function transformForHanaWithCsn(inputModel, options) {
  ...
  let error, warning, info, signal, // alerts
 ...
 // check unique constraints - further processing is done in rewriteUniqueConstraints`
  ...
  function bindCsnReference(){
    ...
    ({ error, warning, info, signal } = alerts(csn, options));
  }
              ...
              if(onConditionPart.ref && (onConditionPart.ref[0] === '$projection' || onConditionPart.ref[0] === '$self')){
                ...
                if(links){
                  ...
                  links.forEach((element, j) => {
                    const { art } = links[j];
                    if(art && art.target)
                      signal(error`Step "${onConditionPart.ref[j]}" in path "${onConditionPart.ref.join('.')}" must not be an association`, path.concat(['on', j]));
                  });
        ...
        if(isUnion(path) && options.toHana){
          if(isBetaEnabled(options, "ignoreAssocPublishingInUnion") && doA2J){
            if((elem.keys || elem.$managed)) {
              signal(info`Managed association "${elemName}", published in a UNION, will be ignored.`, path)
            } else {
              signal(info`Association "${elemName}", published in a UNION, will be ignored.`, path)
            }
            elem._ignore = true;
          }
          else {
            signal(error`Association "${elemName}" cannot be published in a HANA CDS UNION.`, path)
          }
        } else {
          /* Old implementation:
